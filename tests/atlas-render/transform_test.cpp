...
TEST(Lerp, Vec3InterpolatesEachComponentIndependently) {
    const core::Vec3 start{.x = 0.0F, .y = 0.0F, .z = 0.0F};
    const core::Vec3 end{.x = 10.0F, .y = -10.0F, .z = 4.0F};

    const core::Vec3 result = lerp(start, end, 0.5);
...
TEST(Nlerp, AtZeroReturnsFirstRotationNormalized) {
    const core::Quaternion start{.x = 0.0F, .y = 0.0F, .z = 0.0F, .w = 1.0F};
    const core::Quaternion end{.x = 1.0F, .y = 0.0F, .z = 0.0F, .w = 0.0F};

    const core::Quaternion result = nlerp(start, end, 0.0);
...
TEST(Nlerp, AtOneReturnsSecondRotationNormalized) {
    ...
    const core::Quaternion result = nlerp(start, end, 1.0);
...
TEST(Nlerp, MidpointIsNormalized) {
    ...
    const core::Quaternion result = nlerp(start, end, 0.5);
...
TEST(Nlerp, OppositeQuaternionsFallBackToTheSecondRotationRatherThanDividingByZero) {
    ...
    const core::Quaternion start{.x = 0.0F, .y = 0.0F, .z = 0.0F, .w = 1.0F};
    const core::Quaternion end{.x = 0.0F, .y = 0.0F, .z = 0.0F, .w = -1.0F};
