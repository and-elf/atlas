...
// Health/ApplyDamage/HealthChanged) has one or two fields; 32 leaves a 16x
// margin over that before a shape is simply unsupported, at the cost of 32
// cases written out by hand rather than an unbounded number.
//
// Originally capped at 16 (see issue #101): every case is still a distinct,
// textually-written structured-binding declaration — there is no standard
// C++23 way to generate one from a template loop or pack expansion — so
// raising this cap further is always this same mechanical, if tedious,
// extension of detail::tie_fields's if-constexpr chain, not a one-line
// constant change. 32 was chosen as a generous-but-bounded doubling of the
// previous cap rather than an unbounded/macro-generated table, since no
// contract struct in this repository is anywhere near even the old cap yet.
inline constexpr std::size_t max_supported_fields = 32;
...
// A flat, mechanical dispatch chain (one independent, unnested branch per supported field count) is
// exactly the shape max_supported_fields' comment above describes; the cognitive-complexity metric
// below penalizes its branch count as if it were nested control flow, which it isn't - each branch is
// an independent, self-contained case.
