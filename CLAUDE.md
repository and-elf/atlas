...
**This repository is early-stage, but has grown well past its original seed — this file lags the actual codebase by design.** The build/lint/test/coverage scaffolding is in place and enforced end-to-end. Sixteen libraries exist under `libraries/`: the original eleven (`atlas-core`, `atlas-entity`, `atlas-contracts`, `atlas-stage`, `atlas-resource`, `atlas-serialization`, `atlas-reflection`, `atlas-request`, `atlas-scheduler`, `atlas-replication`, `atlas-runtime`), plus five added since — `atlas-render`, `atlas-audio`, `atlas-physics` (backend-swappable, optional presentation/simulation libraries on the same mechanism-not-meaning boundary), `atlas-diagnostics` (§6's uniform runtime-failure-reporting mechanism, not optional the way the other four are), and `atlas-windowing` (shared SDL3 window/context infrastructure `atlas-render`'s and `atlas-input`'s SDL3 backends both opt into). Five tools exist under `tools/`: `atlas-cgen` (manifest-to-C++ contract generator), `atlas-refl` (reflection metadata generation), `atlas-depval` (capability dependency-graph validation), `atlas-docgen` (documentation generation), and `atlas-rcc` (resource compilation). `atlas-input` and `atlas-ui` are seeded (a deliberate minimal-TDD-scaffold scope, not their full eventual surface); `atlas-editor` remains the one genuinely unimplemented library, with a status-stub `README.md` instead. Treat "Repository Layout" below, and every library's own `README.md`, as more current than this paragraph the moment they disagree with it — this file is a condensed guide, not the source of truth on current scope.
...
├── include/
├── src/
├── tools/            # atlas-cgen (contracts), atlas-refl (reflection), atlas-depval (dep-graph validation),
│                     # atlas-docgen (docs), atlas-rcc (resource compilation) — all seeded
...
└── libraries/
    ...
    ├── atlas-diagnostics     # structured logging/diagnostics — §6's uniform failure-channel mechanism
    ...
    ├── atlas-editor          # reusable editor capabilities (optional, not yet implemented)
    ├── atlas-render          # 3D rendering, State → Renderer → Output, backend-swappable (optional)
    ├── atlas-audio           # audio rendering, State → Renderer → Output, backend-swappable (optional)
    ├── atlas-physics         # rigid-body simulation/collision, backend-swappable (optional)
    └── atlas-windowing       # shared SDL3 window/context for atlas-render/atlas-input's SDL3 backends (optional)
```

Before adding code, confirm which library it belongs in (§13 has the full responsibility table) rather than growing an unrelated one. `atlas-editor`, `atlas-input`, `atlas-ui`, `atlas-render`, `atlas-audio`, `atlas-physics`, and `atlas-windowing` are all optional — a headless server host must never gain a dependency on any of them (§13). `atlas-diagnostics` is different in kind, not just degree: it's a general runtime-tier mechanism (§6's uniform failure channel), not a presentation/simulation backend, so it doesn't belong in that same "optional, a server never needs it" bucket.
...
**Naming convention: every platform tool under `tools/` is prefixed `atlas-`** (`atlas-cgen`, `atlas-refl`, `atlas-depval`, `atlas-docgen`, `atlas-rcc`), the same convention libraries already follow. A tool's C++ namespace matches its suffix (`atlas::cgen` for `atlas-cgen`, mirroring `atlas::core` for `atlas-core`). Spec §12 (Atlas Tooling) lists the distinct compile-time responsibilities these five now cover, each its own tool rather than a mode of one monolithic tool: contract generation (`atlas-cgen`), resource compilation — turning authored asset lists into compiled `ResourceId` tables (`atlas-rcc`), reflection metadata generation (`atlas-refl`), capability dependency-graph validation — the cycle-detection compile-time failure required by §5 (`atlas-depval`), and documentation generation (`atlas-docgen`). A generator understanding a vocabulary *type* (e.g. `atlas-cgen` mapping a manifest field typed `ResourceId` to `atlas::ResourceId` and its header) is not the same responsibility as *compiling* that type's backing data — don't conflate the two when deciding which tool a piece of logic belongs in.
